...
const workerPath: URL = new URL(import.meta.url.replace("index.ts", "") + "worker.ts")
...
async function main() {
  ...
  function createWorker(workerId: number) {
    ...
    const workerData: WorkerCreateData = {
      ...
      path: workerPath.toString().replace("file:///", ""),
      totalShards: gateway.manager.totalShards,
      workerId,
    };

    const worker = new Worker(workerPath.toString().replace("file:///", "").replace(".ts", ".js"), {
      workerData,
    });
